...
describe("RepoTable", () => {
    ...
    test("renders no data message", () => {
        ...
        expect(screen.getByText("No Available Data to Display.")).toBeInTheDocument();
    });
    ...
    test("renders repositories", () => {
        ...
        expect(screen.getAllByRole("row")).toHaveLength(mockState.repositories.length + 1); // +1 for header row
    ...
    test("opens repository URL in a new tab", () => {
        ...
        expect(openMock).toHaveBeenCalledWith("https://example.com/repo1", "_blank");
    });
});
